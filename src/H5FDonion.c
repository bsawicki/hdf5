...
#define WIP 1 /* TODO: development toggle */
...
H5FD__onion_update_and_write_whole_history(H5FD_onion_t *file)
{
    ...
    /* TODO: determine destination file */
...
H5FD__onion_commit_new_revision_record(H5FD_onion_t *file)
{
    ...
    /* TODO: replace bogus time-of-creation */
     ...
     * TODO: this is a non-obvious side-effect of the call; can we refactor?
     */
...
#if 0 /* TODO */
        if (record->index_entry_list != NULL) {
            HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, 0,
                    "index entry list prematurely allocated");
        }
#endif
        ...
        haddr_t maxaddr)
{
    ...
    if (H5FD_ONION_STORE_TARGET_H5 == fa.store_target) { /* TODO: test */
        HGOTO_ERROR(H5E_ARGS, H5E_UNSUPPORTED, NULL,
                "same-file storage not implemented");
    }
    else
    if (H5FD_ONION_STORE_TARGET_ONION != fa.store_target ) { /* TODO: test */
    ...
    file->name_recov = (char *)HDmalloc(
            sizeof(char) * (HDstrlen(name_onion) + 10));
    if (NULL == name_onion) {
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                "unable to allocate recovery name string");
    }
    HDsnprintf(file->name_recov, HDstrlen(name_onion) + 10, "%s.recovery",
            name_onion);
    ...
    if ((H5F_ACC_CREAT | H5F_ACC_TRUNC) & flags) {
        ...
        if (NULL == file->backing_onion) {
            if (H5F_ACC_RDWR & flags) {
                /* TODO: instantiate onion history */
            }
            else {
                HGOTO_ERROR(H5E_VFL, H5E_CANTOPENFILE, NULL,
...
#if 0
    /* TODO: this is a crude sanity-check; consider removing */
    if (file->rev_record.archival_index.list == NULL) {
        HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL,
                "null archival index list pointer");
    }
#endif
...
H5FD__onion_open_rw(H5FD_onion_t *file, unsigned int flags, haddr_t maxaddr)
{
    ...
    /* TODO: determine correct file and location (support h5 storage target) */
    if (H5FDwrite(file->backing_onion, H5FD_MEM_DRAW, H5P_DEFAULT, 0,
            (haddr_t)size, buf) < 0)
    {
        HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
        ...
        void *_buf_out)
{
    ...
    /* TODO: initial implementation is naive and slow */
        ...
        const void *_buf)
{
    ...
    for (i = 0; i < n_pages; i++) {
        ...
        if (H5FD_onion_revision_index_find(file->rev_index, page_i, &entry_out_p)) {
            /* TODO: revize page in revindex */
            if (page_gap_head | page_gap_tail) {
                /* Copy existing page verbatim. */
                /* TODO: identify correct target file */
                if (H5FDread(file->backing_onion, H5FD_MEM_DRAW, H5P_DEFAULT,
 ...
 *
 *              TODO: something about file association of entries?
 *
 * Return:      Success: Positive value (1) -- entry found.
 *                       Entry out pointer-pointer is set to point to entry.
 *              Failure: Zero (0) -- entry not found.
